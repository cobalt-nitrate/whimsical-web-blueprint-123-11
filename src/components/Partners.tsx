...
const ProjectCard = ({
  ...
  return <div ref={cardRef} className="glass-card glass-card-hover p-6 rounded-lg opacity-0 h-full flex flex-col">
      <div className="flex flex-wrap gap-2 mb-4 min-h-14">
        <div className=" bg-dark-700 text-teal-500 rounded-full text-xs">
          {industry}
        </div>
        {tech.map(t => <div key={t} className="inline-flex px-3 py-1 bg-dark-700 text-teal-500 rounded-full text-xs">
            {t}
          </div>)}
      </div>
      ...
      <Link to="/case-studies" className="inline-flex items-center text-teal-500 hover:text-teal-400 transition-colors duration-300 text-sm font-medium group mt-auto">
        View Case Study
